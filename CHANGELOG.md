...
- Fixed pre-existing warnings surfaced by the new `-Werror`/`/WX` enforcement across GCC, Clang, and MSVC:
  ...
  - Removed the genuinely-unused-but-previously-kept parameters `basedir`/`num_thr` from `src/common/bitset_dictionary.h`'s `constructdictionary` (updating both call sites), `num_reads_clean` from `src/preprocess/input_preparation.cpp`'s `merge_paired_n_reads`, `deterministic_mode` from `src/reorder/read_reordering_impl.h`'s `writetofile` (declaration, definition, and call site), and `eg` from `src/encode/encoder_impl.h`'s `write_unaligned_range` (both call sites).
  ...
  - Removed the unused `idx` parameter from `src/workflow/archive_preview.cpp`'s `print_gzip_compression_info` (updating all 7 call sites), and removed the unused `read_archive_label` parameter from `src/workflow/decompression_workflow.cpp`'s `materialize_aliased_group_output_from_memory`.
  ...
  - Removed the unused `estimated_input_bytes` parameter from `src/workflow/workflow_shared.cpp`'s `estimate_memory_path_safety_margin_bytes` (updating its call site), and fixed a GCC `-Wrange-loop-construct` warning by iterating `default_archive_name_from_input`'s extension list as `const char *` instead of binding a `const std::string &` to a temporary.
