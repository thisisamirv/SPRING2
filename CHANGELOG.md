...
- Disabled IPO/LTO on Apple platforms (`APPLE`). Apple Clang + ld64 ThinLTO has been observed to turn a caught `std::runtime_error` (from decoding a deliberately corrupted archive) into a SIGSEGV instead of a clean error message, specifically on the `macOS x64 Clang` CI leg. This mirrors the existing `-fno-lto` workaround already applied to `vendor/cloudflare_zlib`.
