...
- Fixed `-t`/`--threads` having no effect during decompression. SPRING2 previously used the archive's encoding thread count for reconstruction and output compression regardless of the requested value; the option now controls decompression parallelism while the encoded thread count remains responsible only for interpreting the archive's block layout.
...
- Suppressed ClangCL-only padding and C++-keyword warnings from vendored ISA-L without changing its public structure layouts or C compatibility code.
- Fixed ClangCL decompression failures for chunked archives by replacing requested-thread-based manual chunk partitioning with OpenMP work sharing and stable pointers to packed and decoded chunk storage.
- Hardened benchmark downloads against transient ENA HTTP failures with a descriptive user agent, bounded retries, transfer-size validation, and published MD5 checksums for SRR8185389.
...
- Simplified the public `decompress()` API by dropping the unused `compression_level` parameter.
